"""
La idea de un verificador eficiente debe ser que dada una solucion a un problema, se corobore de manera polinomial
...
def verificador_eficiente(tablero_solucion, barcos, demandas_filas, demandas_columnas):
    ...
    - Lista de barcos original (cada indice i deberia ser el largo del barco i+1).
    ...
    - Los barcos sean los mismos que los de la lista original de barcos.
    - Los barcos cumplan con la ley de adyacencia.
    - No hayan barcos de mas.

    Pos: 
    - Devuelve True en caso de que sea una solucion valida y False en caso contrario.
    """
    ...
    for fila in range(len(tablero_solucion)):
        for columna in range(len(tablero_solucion[0])):
            if tablero_solucion[fila][columna] != 0 and not tablero_solucion[fila][columna] in barcos_checkeados:
                ...
                if barco < len(barcos):
                    # Si esta en el rango de barcos, sera el largo que deberia tener originalmente.
        ...
        if columna + barco_largo <= columnas:
            if all(tablero[fila, columna:columna + barco_largo] == barco):
                ...
                if all(tablero[max(0, fila - 1):min(filas, fila + 2), max(0, columna - 1):min(columnas, columna + barco + 1)].flatten() == 0):
                    # Verificar adyacencia. (Si el largo es mayor al que deberia el barco, falla aca).
                    return True
